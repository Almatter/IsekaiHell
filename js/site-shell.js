...
$(function(){
  ...
  $(document).on('mouseenter focusin','.ui-btn,.site-link,.lore-nav-button,.country-card,.race-directory button,.race-tree-button,.race-tree-card,summary',function(){const now=performance.now();if(now-lastHover>75){lastHover=now;sound('hover')}}).on('pointerdown','.ui-btn,.site-link,.lore-nav-button,.country-card,.race-directory button,.race-tree-button,.race-tree-card,summary',()=>sound('click'));
  window.IHSiteSound={sound,setTrack,setContext,setGroup,getPrefs:()=>({...prefs})};
});
})(jQuery);
